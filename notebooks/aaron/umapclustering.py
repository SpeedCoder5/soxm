...
# Iterating over each group to display the clustered data
for cluster_num, group in grouped:
    # print(f"Cluster {cluster_num}:")
    for index, row in group.iterrows():
        # print(f"Model: {row['Model']}, Topic: {row['Topic']}, Prompt: {row['Prompt']}, Response: {row['Response']}")
        responses = ' '.join(row['Response'])
        prompt = responses + " Please summarize the above text into a main topic in 4 words or less"
        topic = get_openai_response(prompt, 'gpt-3.5-turbo')
        print(topic)
    print("\n")